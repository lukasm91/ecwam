...
! ----------------------------------------------------------------------
      ...
      REAL(KIND=JWRB), DIMENSION(KIJL),INTENT(IN):: DEPTH, EMAXDPT, EMEAN, F1MEAN


      INTEGER(KIND=JWIM) :: IJ, K, M, IC
      REAL(KIND=JPHOOK) :: ZHOOK_HANDLE
      REAL(KIND=JWRB) :: ALPH, ARG, Q, Q_OLD, REL_ERR, EXPQ
      REAL(KIND=JWRB),DIMENSION(KIJL) :: SDS
...
!            USE NEWTON-RAPHSON METHOD
      ...
      SUBROUTINE SDIWBK_PW(IDX, KIJL, FL1, FLD, SL, DEPTH, EMAXDPT, EMEAN, F1MEAN)
!$loki routine seq
