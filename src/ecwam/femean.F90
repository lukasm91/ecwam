...
!        ------------------------------------------
      ...
      SUBROUTINE FEMEAN_PW (IDX, KIJS, KIJL, F, EM, FM)
!$loki routine seq
      USE PARKIND_WAVE, ONLY : JWIM, JWRB, JWRU
...
! ----------------------------------------------------------------------
      ...
      REAL(KIND=JWRB), INTENT(OUT) :: EM, FM
...
!        ------------------------------------------
      ...
      DO M=1,NFRE
        K=1
        TEMP2 = MAX(F(IDX,K,M), EPSMIN)
        DO K=2,NANG
          TEMP2 = TEMP2+ MAX(F(IDX,K,M), EPSMIN)
        ENDDO
        EM = EM+TEMP2*DFIM(M)
