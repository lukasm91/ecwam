...
! ----------------------------------------------------------------------
      ...
      REAL(KIND=JWRB), INTENT(IN) :: PHIWA
      REAL(KIND=JWRB), INTENT(IN) :: EM, F1, WSWAVE, WDWAVE
      REAL(KIND=JWRB), INTENT(IN) :: UFRIC, AIRD
      REAL(KIND=JWRB), INTENT(INOUT) :: TAUXD, TAUYD, TAUOCXD, TAUOCYD, TAUOC
      REAL(KIND=JWRB), INTENT(INOUT) :: PHIOCD, PHIEPS, PHIAW
      REAL(KIND=JWRO), INTENT(INOUT) ::  NPHIEPS, NTAUOC, NSWH, NMWP, NEMOTAUX
      REAL(KIND=JWRO), INTENT(INOUT) ::  NEMOTAUY, NEMOWSWAVE, NEMOPHIF
      LOGICAL, INTENT(IN) :: LNUPD
...
!     THE INTEGRATION ONLY UP TO FR=MIJ
        DO M=1,NFRE
          ...
          ENDDO
          PHILF   = PHILF   + SUMT*RHOWGDFTH(IDX,M)
          CMRHOWGDFTH = CINV(IDX,M)*RHOWGDFTH(IDX,M)
          XSTRESS = XSTRESS + SUMX*CMRHOWGDFTH
          YSTRESS = YSTRESS + SUMY*CMRHOWGDFTH
        ENDDO

        IF (LICERUN .AND. LWAMRSETCI) THEN
          IF(CICOVER > CIBLOCK) THEN
            OOVAL=EXP(-MIN((CICOVER*CITHRSH_INV)**4,10._JWRB))
!           ADJUST USTAR FOR THE PRESENCE OF SEA ICE
            U10P = MAX(WSWAVE,EPSU10)
            CD_BULK = MIN((C1 + C2*U10P**P1)*U10P**P2, CDMAX)
            CD_WAVE = (UFRIC/U10P)**2
        ...
        TAU        = AIRD*MAX(USTAR**2,EPSUS)
        TAUXD  = TAU*SIN(WDWAVE)
        TAUYD  = TAU*COS(WDWAVE)

        TAUOCXD= TAUXD-OOVAL*XSTRESS
        TAUOCYD= TAUYD-OOVAL*YSTRESS
        TAUO               = SQRT(TAUOCXD**2+TAUOCYD**2)
        TAUOC  = MIN(MAX(TAUO/TAU,TAUOCMIN),TAUOCMAX)

        XN                = AIRD*MAX(USTAR**3,EPSUS3)
        PHIOCD= OOVAL*(PHILF-PHIWA)+(1.0_JWRB-OOVAL)*PHIOC_ICE*XN

        PHIEPS= PHIOCD/XN 
        PHIEPS= MIN(MAX(PHIEPS,PHIEPSMIN),PHIEPSMAX)

        PHIOCD= PHIEPS*XN

        PHIAW = PHIWA/XN
        PHIAW = OOVAL*PHIWA/XN+(1.0_JWRB-OOVAL)*PHIAW_ICE

        IF (LWNEMOCOU .AND. LNUPD) THEN
          NPHIEPS = PHIEPS
          NTAUOC  = TAUOC
          IF (EM_OC/=0.0_JWRB) THEN
             NSWH = 4.0_JWRO*SQRT(EM_OC)
          ELSE
             NSWH = 0.0_JWRO
          ENDIF
          ...
          IF (LWNEMOTAUOC) THEN
            NEMOTAUX = NEMOTAUX + TAUOCXD
            NEMOTAUY = NEMOTAUY + TAUOCYD
          ELSE
            NEMOTAUX = NEMOTAUX + TAUXD
            NEMOTAUY = NEMOTAUY + TAUYD
          ENDIF 
          NEMOWSWAVE = NEMOWSWAVE + WSWAVE
          NEMOPHIF   = NEMOPHIF + PHIOCD 
        ENDIF

END SUBROUTINE WNFLUXES_PW
